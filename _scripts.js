...
async function performAction(rawArgs) {
    ...
    if (firstArg == "contracts:dev") {
        ...
        await execute(`dotenv -e .env -e contracts/.env -- npm --prefix contracts run local:dev -- --export ../web/src/lib/contracts.json`);
    } else if (firstArg === "contracts:deploy") {
        const { fixedArgs, extra } = parseArgs(args, 1, {});
        const network = fixedArgs[0] || "localhost";
        const env = getEnv(network);
        await execute(`${env}npm --prefix contracts run deploy ${network} -- --export ../web/src/lib/contracts.json ${extra.join(" ")}`);
