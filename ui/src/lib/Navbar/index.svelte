<script>
    ...
    const connectWallet = () => address === `` ? modal = true : ``;

    const close = (provider) => {
        modal = false;
        window.web3.eth.getAccounts().then((account) => {
            address = account.toString();
            prefix = address.substring(0, 8);
            console.log(walletButton.classList);
    ...
    <Container>
        <Level>
            ...
            <div slot="right">
                <Menus />
                <Theme />
                <div bind:this={walletButton} class="button">
                    <Button text="{prefix === `` ? `Connect Wallet` : prefix}" action={connectWallet} />
                </div>
            </div>
        </Level>
    </Container>
...
<style>
    ...
    :global {
        ...
        nav .button.connected {
            ...
            border: 3px solid #b7e4c7 !important;
        }
    }
</style>