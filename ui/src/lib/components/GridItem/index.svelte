<script>
    ...
    const imageHash = vault.imageHash;
    ...
    if (vault.protocol === `Yearn`) {
        const us = new Intl.NumberFormat(`en-US`);
        yearn().then((yVaults) => {
            yVaults.forEach((yVault) => {
                if ((vault.name === `LINK` && yVault.inception === 12561326) || (vault.name === `DAI` && yVault.inception === 11674216) || (vault.name === `WBTC` && yVault.inception === 12215420) || (vault.name === `ETH` && yVault.inception === 12589035)) {
                    apr = (yVault.apy.recommended * 100).toFixed(2);
                    tvl = `$${us.format(yVault.tvl.value)}`;
                    apm = (apr / 12).toFixed(2);
                    apw = (apr / 52).toFixed(2);
                    apd = (apr / 365).toFixed(2);
                }
            });
        });
    }
    ...
    const deposit = (amount) => {
        if ($wallet.address !== undefined) {
            ...
            flow.execute(async (contracts) => {
                await contracts.Strategy.deposit(ethers.utils.parseEther(`1`), {
                    value: ethers.utils.parseEther(`1`),
                });
                deposited++;
                provider.getBalance($wallet.address).then((res) => available = ethers.utils.formatEther(res));
            });
        } else {
    ...
    const withdraw = (amount) => {
        if ($wallet.address !== undefined) {
            flow.execute(async (contracts) => {
                await contracts.Strategy.withdraw(ethers.utils.parseEther(`1`));
            });
        } else {
            alert(`Please connect your wallet first!`);
        }
    ...
    <div class="content">
        <Level>
            <div class="group" slot="left">
                <div class="icon">
                    <img src="/img/ipfs/{imageHash}.svg" alt="{name}" loading="lazy">
                </div>
                <div>
                    <span class="title">{name}</span>
                    <br>
                    <span class="subtitle">APR {apr}%</span>
                </div>
            </div>
        ...
        <div bind:this={stats}>
            <Level>
                ...
                <div class="shrink" slot="right"><span class="value">{tvl}</span></div>
            </Level>
            <Level>
                <div class="shrink" slot="left"><span class="key">Deposited</span></div>
                <div class="shrink" slot="right"><span class="value">{deposited}</span></div>
            </Level>
            <Level>
                <div class="shrink" slot="left"><span class="key">Available</span></div>
                <div class="shrink" slot="right"><span class="value">{available}</span></div>
            </Level>
            <div class="bottom">
                {#if vault.name === `ETH`}
                    <Level>
                        <div slot="left">
                            <button on:click={() => deposit(1)}>Deposit</button>
                        </div>
                        <div slot="right">
                            <button on:click={() => withdraw(1)}>Withdraw</button>
                        </div>
                    </Level>
                {:else}
                    <button on:click={approve}>Approve {name}</button>
                {/if}
            </div>
        </div>
        <div bind:this={roi} style="display: none;">
            <Level>
                ...
                <div class="shrink" slot="right"><span class="value">{apd}%</span></div>
            </Level>
            <Level>
                <div class="shrink" slot="left"><span class="key">7D</span></div>
                <div class="shrink" slot="right"><span class="value">{apw}%</span></div>
            </Level>
            <Level>
                <div class="shrink" slot="left"><span class="key">30D</span></div>
                <div class="shrink" slot="right"><span class="value">{apm}%</span></div>
            </Level>
            <Level>
                <div class="shrink" slot="left"><span class="key">365D</span></div>
                <div class="shrink" slot="right"><span class="value">{apr}%</span></div>
            </Level>
        </div>
    </div>
</div>
    ...
    :global {
        .item .bottom {
            button {
                ...
                width: 100%;
                &:hover {
                    background-color: #d3f8d8 !important;
                }
            }
            & > button {
